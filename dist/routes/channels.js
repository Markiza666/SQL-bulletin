...
router.post('/', async (req, res) => {
    ...
    try {
        const result = await pool.query('INSERT INTO channels (name, owner_id) VALUES ($1, $2) RETURNING *', [name, owner_id]);
...
router.get('/:id/messages', async (req, res) => {
    ...
    try {
        const result = await pool.query(`SELECT m.*, u.username
            FROM messages m
            JOIN users u ON m.user_id = u.id
            JOIN message_channels mc ON m.id = mc.message_id
            WHERE mc.channel_id = $1
            ORDER BY m.created_at`, [channelId]);
